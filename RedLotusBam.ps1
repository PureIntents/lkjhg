...
# Buscar archivo.exe en Documentos, Documents y OneDrive de todos los usuarios
$filePath = $null
...
if (!$filePath) {
    $oneDrivePath = Get-ChildItem -Path $env:OneDrive -Directory -Recurse | Where-Object { $_.Name -eq "Documentos" } | Select-Object -First 1
    if ($oneDrivePath) {
        $filePath = Get-ChildItem -Path (Join-Path -Path $oneDrivePath.FullName -ChildPath "launcher.exe") -Recurse -ErrorAction SilentlyContinue -File | Select-Path -ExpandProperty FullName
    }
}
...
if ($filePath) {
    ...
    Write-Host "El archivo launcher.exe no se encontró en Documentos, Documents o OneDrive de ningún usuario"
}
